...
package org.puremvc.as3.multicore.utilities.fabrication.patterns.command
{
	...
	public class SimpleFabricationCommand extends SimpleCommand implements ICommandProcessor, IDisposable {
		...
		public function registerCommand( notificationName:String, commandClassRef:Class ):void {
			if ($mvcLogger.debugEnabled) $mvcLogger.debug("registerCommand [command: {0}, class: {1}]",notificationName,getQualifiedClassName(commandClassRef));
		...
		public function removeCommand( notificationName:String ):void {
			if ($mvcLogger.debugEnabled) $mvcLogger.debug("removeCommand [command: {0}]",notificationName);
		...
		public function registerProxy( proxy:IProxy ):IProxy {
			if ($mvcLogger.debugEnabled) $mvcLogger.debug("registerProxy [name: {0}]",proxy.getProxyName());
		...
		public function removeProxy( proxyName:String ):IProxy {
			if ($mvcLogger.debugEnabled) $mvcLogger.debug("removeProxy [name: {0}]",proxyName);
		...
		public function registerMediator( mediator:IMediator ):IMediator {
			if ($mvcLogger.debugEnabled) $mvcLogger.debug("registerMediator [name: {0}]",mediator.getMediatorName());
		...
		public function removeMediator( mediatorName:String ):IMediator {
			if ($mvcLogger.debugEnabled) $mvcLogger.debug("removeMediator [name: {0}]",mediatorName);
		...
		public function routeNotification(noteName:Object, noteBody:Object = null, noteType:String = null, to:Object = null):void {
			if ($mvcLogger.debugEnabled) $mvcLogger.debug("routeNotification [name: {0}, type: {1}, to: {2}]",noteName,noteType,to);
		...
		public function registerInterceptor(noteName:String, clazz:Class, parameters:Object = null):void {
			if ($mvcLogger.debugEnabled) $mvcLogger.debug("registerInterceptor [name: {0}, class: {1}]",noteName,getQualifiedClassName(clazz));
		...
		public function removeInterceptor(noteName:String, clazz:Class = null):void {
			if ($mvcLogger.debugEnabled) $mvcLogger.debug("removeInterceptor [name: {0}, class: {1}]",noteName,getQualifiedClassName(clazz));
