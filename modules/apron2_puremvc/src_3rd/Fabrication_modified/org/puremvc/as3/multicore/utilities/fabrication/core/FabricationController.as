...
package org.puremvc.as3.multicore.utilities.fabrication.core {
	...
	public class FabricationController extends Controller implements IDisposable {
		 ...
		 * 
		 * @param noteName The name of the notification to register the interceptor with.
		 * @param noteType The name of the type notification to register the interceptor with.
		 * @param clazz The concrete interceptor class.
		 * @param parameters Optional parameters for the interceptor.
		 */
		public function registerInterceptorByType(noteName:String, noteType:String, clazz:Class, parameters:Object = null):void 
		{
			registerInterceptor(noteName+"__"+noteType,clazz,parameters);
		}
		 ...
		 * @param clazz The name of the concrete interceptor class that was registered earlier with this noteName.
		 */
		public function removeInterceptorByType(noteName:String, noteType:String, clazz:Class = null):void
		{
			removeInterceptor(noteName+"__"+noteType,clazz);
		}
		 ...
		 * @param noteName The name of the notification whose mapping is to be looked up. 
		 */
		public function hasInterceptorByType(noteName:String, noteType:String):Boolean
		{
			return hasInterceptor(noteName+"__"+noteType);
		}
		...
		public function intercept(note:INotification):Boolean {
			...
			if (hasInterceptor(noteName)) key = noteName;
			if (hasInterceptorByType(noteName,noteType)) key = noteName+"__"+noteType;

			if (!key && (noteName == RouterNotification.SEND_MESSAGE_VIA_ROUTER ||
						 noteName == RouterNotification.RECEIVED_MESSAGE_VIA_ROUTER))
			{
				var transport:TransportNotification = note.getBody() as TransportNotification;
				
				if (!transport) return false;
				if (transport.getCustomNotification())
				{
					noteName = transport.getCustomNotification().getName().toString();
					noteType = transport.getCustomNotification().getType();
				}
				else
				{
					noteName = transport.getName() as String;
					noteType = transport.getType() as String;
				}
			...
			if (key)
			{
				mappings = interceptorMappings.find(key) as Array;
